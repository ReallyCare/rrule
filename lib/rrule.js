...
;(function (root, factory) {
  ...
  var dateutil = {
    ...
    getYearDay: function (date) {
      ...
      var dateNoTime = dateutil.createDate(
        date.getFullYear(), date.getMonth(), date.getDate(), timezone
        )
      var dateStartYear = dateutil.createDate(date.getFullYear(), 0, 1, timezone)
      return Math.ceil(
        (dateNoTime - dateStartYear) / dateutil.ONE_DAY) + 1
    },
    ...
    daysBetween: function (date1, date2) {
      ...
      var msDate1 = date1.getTime() - dateutil.tzOffset(date1)
      var msDate2 = date2.getTime() - dateutil.tzOffset(date2)
      // Calculate the difference in milliseconds
      var msDifference = Math.abs(msDate1 - msDate2)
      // Convert back to days and return
      return Math.round(msDifference / dateutil.ONE_DAY)
    },
    ...
    combine: function (date, time) {
      time = time || date
      // TODO(ported from aramk) Not sure why, but Time also has a timezone argument associated with it.
  ...
  var RRule = function (options, noCache) {
    ...
    if (!opts.dtstart) opts.dtstart = dateutil.createDate(undefined, opts.timezone)
  ...
  RRule.enableTimezones = function (DateClass) {
    if (DateClass) {
      dateutil.Date = DateClass
    } else {
      // If class is not specified, assume it is "built in" timezones
      dateutil.Date = getTz().getTzClass()
    }
  ...
  RRule.prototype = {
    ...
    all: function (iterator) {
      var args = {rrule: this instanceof RRuleSet ? this._rrule[0] : this}
      if (iterator) {
    ...
    between: function (after, before, inc, iterator) {
      var args = {
        ...
        after: after,
        inc: inc,
        rrule: this instanceof RRuleSet ? this._rrule[0] : this
      }
    ...
    before: function (dt, inc) {
      var args = {dt: dt, inc: inc, rrule: this instanceof RRuleSet ? this._rrule[0] : this}
    ...
    /*
    Semi-expose createDate (only accept one date parameter - and the timezone)
     */
    createDate: function () {
      var args = Array.prototype.slice.call(arguments)
      return dateutil.createDate(args[0], args[1])
    },
    /**
    ...
    */
    ...
    _iter: function (iterResult) {
      ...
      */
      ...
      var tzstart = dateutil.createDate(dtstart, this.options.timezone)
      var year = tzstart.getFullYear()
      var month = tzstart.getMonth() + 1
      var day = tzstart.getDate()
      var hour = tzstart.getHours()
      var minute = tzstart.getMinutes()
      var second = tzstart.getSeconds()
      var weekday = dateutil.getWeekday(tzstart)
  ...
  IterResult.prototype = {
    ...
    init: function (method, args) {
      ...
      this.maxDate = null
      this._result = []
      var timezone = args.rrule instanceof RRule ? args.rrule.options.timezone : null
      ...
      if (method === 'between') {
        this.maxDate = args.inc
          ? before : dateutil.createDate(args.before.getTime() - 1, timezone)
        this.minDate = args.inc
          ? after : dateutil.createDate(args.after.getTime() + 1, timezone)
     ...
     */
    add: function (date) {
      var timezone = this.args.rrule instanceof RRule ? this.args.rrule.options.timezone : null
  ...
  // TODO Currently no real attempt to support timezones in RRuleSets
  ...
  function getTz () {
    if (!getTz._tz) {
      if (root && root._getRRuleTZ) {
        getTz._tz = root._getRRuleTZ(RRule)
      } else if (typeof require === 'function') {
        getTz._tz = require('./tz')(RRule)
      } else {
        throw new Error('You need to include rrule/tz.js for enableTimezones to work.')
      }
    }
    return getTz._tz
  }
